...
namespace AIClass
{
    class InfoBox
    {
        ...
        public enum sideMenu { main, stats, createMap, viewPath, agentPath, clickSeek };
        ...
        public InfoBox(Rectangle bounds, Player player)
        {
            ...
            menuButtons.Add(new Button(new Rectangle(bounds.X + 10, bounds.Y + 65, 105, 25), "Create Map"));
        ...
        public void respondToInput(InputHandler input, PathGraph navMap)
        {
            ...
            if (addMode && input.mouseClicked(new Rectangle(0, 0, bounds.X, bounds.Height)))
            {
                ...
                Game1.agents.Add(new Agent(input.mouseStateCurrent.X, input.mouseStateCurrent.Y, 24, 24, randomNum));
                addMode = false;
            }
        ...
        public void draw(SpriteBatch spriteBatch, List<Agent> adjacentAgents)
        {
        ...
        public void drawStats(SpriteBatch spriteBatch, List<Agent> adjacentAgents, int infoX, int infoY)
        {
            ...
            for (int i = 0; i < adjacentAgents.Count; i++)
            {
                ...
                spriteBatch.DrawString(font12, "H: " + (MathHelper.ToDegrees(player.getRelativeAngleBetween(adjacentAgents[i].position))).ToString("0.00"), new Vector2(infoX + 130, infoY), Color.White);
            }
